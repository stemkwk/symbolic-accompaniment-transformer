...
### C. 추론 시점 제어 메커니즘 3종 (재학습 불필요 Logits 조작)
* **기존 모델의 한계**: 일반적인 시퀀스 모델은 추론 중 단순 Temperature나 Top-p 조절만으로는 멜로디 추종 강도·화음 두께·화성 충돌을 제어할 수 없습니다. 고착화되면 매개변수를 바꿔 재학습해야 합니다.
* **본 프로젝트의 차별성**: 모델 재학습 없이 **추론 시점 logits 조작만으로 세 축을 독립 제어**합니다 (2026-05-31 기준):
  1. **Structural Suppression** — 다성부 밀도(Polyphony Rate)를 단선율~재즈 화음까지 조절.
  2. **Classifier-Free Guidance (CFG, `cfg_w`)** — 멜로디 조건 추종 강도를 `uncond + w·(cond−uncond)` 블렌딩으로 조절 (학습 시 condition-dropout으로 무조건부 모드를 함께 학습).
  3. **Avoid-note Soft Penalty** — 생성 코드 대비 기피음(예: 메이저 3도 위 11음)의 logit을 부드럽게 감점해 화성 충돌을 억제(색채음·경과음은 보존).
  
  → 단일 추론 변수가 아니라 **밀도·조건성·화성**의 3축을 분리 제어하는, 도메인 지식과 결합된 제어 가능한(Controllable) 생성기입니다.
