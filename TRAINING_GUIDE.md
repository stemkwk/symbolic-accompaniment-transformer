...
## 부록 B. 학습된 모델로 반주 생성 (추론)

> 🎬 **웹 화면에서 클릭만으로 시연**하려면 별도 문서 **[`INFERENCE_GUIDE.md`](INFERENCE_GUIDE.md)** 를 따라 하세요.
> (브라우저에서 멜로디 업로드 → 반주 WAV 듣기. 시연·발표용으로 가장 쉽습니다.)

명령줄로 한 번에 파일을 만들고 싶을 때 (소유자용):
